...
public class ContentFromDatabase {
    ...
    public static void setStep(Step step, IResult<Integer> result) {
        mManager.add(step, result);
    }

    public static void getStepById(int idStep, IResult<List<Step>> result){
        mManager.query(new Step(), StepsTable.PROJECTION, new String[]{ID_STEP},
                new String[]{String.valueOf(idStep)}, result);
    }

    public static void getAllSteps(IResult<List<Step>> result){
        mManager.getAll(new Step(), result);
    }

    public static void updateStep(Step step, IResult<Integer> result){
        mManager.update(step, STEP_SELECTION, new String[]{String.valueOf(step.getId())}, result);
    }

    public static void removeStep(Step step, IResult<Integer> result) {
        mManager.remove(step, result);
    }

    public static void removeAllSteps(IResult<Integer> result) {
        mManager.removeAll(new Step(), result);
    }

    public static void setUser(JUserInfo user, IResult<Integer> result){
    ...
    public static void getUserByNameAndUrl(String userName, String url, IResult<List<JUserInfo>> result) {
